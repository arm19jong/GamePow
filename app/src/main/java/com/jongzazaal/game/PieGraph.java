...
public class PieGraph extends View implements HoloGraphAnimate {
    ...
    public PieGraph(Context context, AttributeSet attrs, int defStyle) {
        ...
        //TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable., 0, 0);
    ...
    public void animateToGoalValues() {
        if (android.os.Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB_MR1){
            Log.e("HoloGraphLibrary error", "Animation not supported on api lew. Returning without animating.");
            return;
        }
        if (mValueAnimator != null)
            mValueAnimator.cancel();
